...
def prepare(source, output, upstream=None):
    source=Path(source).resolve();run=Path(output).resolve()
    c=json.loads(source.read_text());old=Path(c['output']).resolve()
...
def main():
    ...
    p.add_argument('--run',required=True);p.add_argument('--upstream')
    a=p.parse_args();prepare(a.from_config,a.run,a.upstream)
